...
The modeler SHALL provide an authenticated workspace that loads projects for the selected client, allows an authorized user to create and select a project, and displays each project's model type, stable key, display name, ownership, and lifecycle state. Project and version navigation SHALL never expose another client's data.
...
The modeler SHALL expose publication only for an explicitly selected, successfully validated version, send the authenticated publication request with its client and correlation context, and display active, failed, and transport-failure outcomes without falsely marking a local version as published.
