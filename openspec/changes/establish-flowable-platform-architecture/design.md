...
The modeler frontend uses an IDE-like shell with project navigation, model tabs, central canvas, properties/metadata panel, validation output, version history, and publish status. BPMN editing uses `bpmn-js`, verified as an actively maintained browser modeler with current releases, recent upstream activity, BPMN 2.0 editing, and XML import/export through its `bpmn-moddle` foundation. The first-release BPMN editor surface is intentionally minimal: start/end/terminate events, user tasks, approved built-in service tasks, exclusive/parallel gateways, sequence flows, supported timers, user/group assignments, process variables, and references to the versioned generic form schema. Inclusive/event-based gateways, message/signal/escalation events, call activities, event subprocesses, arbitrary scripts/delegates, and unsupported expression features remain unavailable and are rejected by backend validation.

The original BPMN XML remains the source of truth. Import/export must preserve unknown Flowable extension elements and attributes, including extensions the editor does not expose in its properties panel; editor updates may modify known modeled elements but must not silently discard unrecognized XML. This preservation requires the appropriate `bpmn-moddle` extension descriptors and explicit representative round-trip tests; it is not accepted as an untested assumption about the base editor.

For CMMN, the first release uses a constrained custom authoring surface backed by the CMMN 1.1 `cmmn-moddle` XML model/parser, with Flowable runtime validation as the compatibility gate. The related `cmmn-js` renderer/editor and `cmmn-moddle` upstream repositories are archived, so `cmmn-js` is not treated as a maintained product dependency. The editor exposes only stages, human tasks, sentries with the approved condition subset, milestones, supported timers, case variables, and user/group assignments. Unknown CMMN and Flowable extension elements and attributes remain preserved in the source XML during round trips. The backend rejects unsupported constructs before publication.
