...
public class LocalTokenService {
    ...
    public LocalTokenService(@Value("${flowableplus.security.local-token-lifetime:PT1H}") Duration tokenLifetime) {
    ...
    public TokenResponse issue(String username, String password) {
        ...
        tokens.put(token, new IssuedToken(user, Instant.now().plus(tokenLifetime)));
